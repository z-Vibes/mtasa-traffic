...
addEventHandler ( "onClientResourceStart", getResourceRootElement(), 
	function()
		...
		outputChatBox("Resource newtraffic started", 0, 255, 0)
	end
)
...
addEventHandler ( "onClientPreRender", _root, function ()
	...
	for i, ped in ipairs ( getElementsByType ( "ped", getResourceRootElement(), true ) ) do
		if _peds[ped] and _peds[ped].parked ~= true then
			local vehCheck = getElementParent(ped)
			if isElement(vehCheck) then
				local tEnabled = getElementData(vehCheck, "traffic_enabled")
				if tEnabled == false then
					-- this vehicle has traffic disabled (e.g. a rammer); skip processing
					goto continue_ped_loop
				end
			end
